...
while true; do
  conditions=$(gcloud beta run domain-mappings describe --platform managed --domain "$SUBDOMAIN.$ROOT_DOMAIN" --region "$GOOGLE_REGION" --format json --project "$GOOGLE_PROJECT" | jq '.status.conditions')
